...
use crate::{AnalysisManager, KernelPatternAdapter, KernelRegistry, Pass, PassContext, PassError};

use ir::{GreedyRewriteConfig, Module, RewritePatternSet, apply_patterns_greedily};

pub const SELECT_CUSTOM_KERNELS_PASS: &str = "kernel.select-custom";
...
impl Pass for SelectCustomKernelsPass {
    ...
    fn run(
        ...
        for registration in self.registry.registrations() {
            if !registration.supports(context.request().target()) {
                continue;
            }

            let pattern = registration.pattern();
