...
pub const KERNEL_DIALECT: &str =
    "kernel";

pub const KERNEL_CALL_OPERATION: &str =
    "kernel.call";

pub const KERNEL_NAME_ATTRIBUTE: &str =
    "kernel.name";

pub const KERNEL_SYMBOL_ATTRIBUTE: &str =
    "kernel.symbol";

pub const KERNEL_BACKEND_ATTRIBUTE: &str =
    "kernel.backend";

pub const KERNEL_PARAMETER_COUNT_ATTRIBUTE:
    &str =
    "kernel.parameter_count";

pub const KERNEL_RESULT_COUNT_ATTRIBUTE:
    &str =
    "kernel.result_count";

pub const KERNEL_RESULT_ALIAS_PREFIX:
    &str =
    "kernel.result_alias.";
...
pub fn register_kernel_dialect(
    registry:
        &mut DialectRegistry,
) -> Result<
    (),
...
fn verify_kernel_call(
    operation: OperationRef<'_>,
) -> Result<(), String> {
    if !operation.regions().is_empty() {
        return Err(
            "kernel.call must not own regions"
                .into(),
        );
    }
    ...
    if !operation.successors().is_empty() {
        return Err(
            "kernel.call must not have successors"
                .into(),
        );
    }
    ...
    let result_count =
        match operation.attribute(
            ...
            Some(
                ...
                usize::try_from(*value)
                    .map_err(|_| {
                        "kernel result count is too large"
                            .to_owned()
                    })?
            }
